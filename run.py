...
M = np.load(os.path.join(curr_dir,'scripts/ImageCap/embedB.npy'))
cap_seq = np.load('scripts/ImageCap/caption_vec.npy')

word_ind_map=dict()
with open(os.path.join(curr_dir,'scripts/ImageCap/word_ind_map.pkl'), 'rb') as f:
  word_ind_map = pickle.load(f)

ind_word_map=dict()
with open(os.path.join(curr_dir,'scripts/ImageCap/ind_word_map.pkl'), 'rb') as f:
  ind_word_map = pickle.load(f)

IV3_feat = tf.keras.models.load_model(os.path.join(curr_dir,'scripts/ImageCap/IV3_feat.h5'))

embed_dim = 300
units = 512
enc = ENCODER(embed_dim)
dec = DECODER(units, M, 80)
enc.load_weights(os.path.join(curr_dir,'scripts/ImageCap/models/encoder'+str(5)+'/'))
dec.load_weights(os.path.join(curr_dir,'scripts/ImageCap/models/decoder'+str(5)+'/'))
